...
class R_Metabox {
	...
	public function __construct( $options, $box ) {
		...
		/* Set URI path */
		if ( $this->box['admin_url'] == '' ) {
			$_path_uri = get_template_directory();
		} else {
			$_path_uri = $this->box['admin_url'];
		}
		$this->admin_url = $_path_uri . $this->box['admin_dir'];
		...
		if ( ! function_exists( 'mr_image_resize' ) )
			include_once( $this->admin_url . '/functions/mr-image-resize.php' );
	...
	/* Image exist
	---------------------------------------------- */
	private function get_image( $img ) {
	...
	/* Image resize
	---------------------------------------------- */
	private function img_resize( $width, $height, $src, $crop = 'c', $retina = false ) {
	...
	private function add_image( $value ) {
		...
		if ( isset( $value['id'][0]['std'] ) && $value['id'][0]['std'] != '' && $theme_path == false ) {
				...
				if ( $by_id ) {
					...
					$image = $this->get_image( $value['id'][0]['std'] );
				}

				// If image exists
				if ( $image ) {
					echo '<img src="' . $this->img_resize($value['width'], $value['height'], $value['id'][0]['std'], $preview_crop) . '" alt=" ' . $value['id'][0]['std'] . ' ">';
				} else {
					$display = 'block';
					$del_display = 'none';
					$crop_display = 'none';
	...
	function thumb_generator() {
		...
		if ($img)
			$this->e_esc( $this->img_resize($width, $height, $id, $crop ) );
		else
			echo 'error';
		exit;
	}
	...
	private function bg_generator_box() {
		...
		<label for="bg-repeat">' . _x('Repeat', 'Metabox Class', $this->textdomain ) . '</label>
	          ...
	          <p class="help-box">' . _x('The background-repeat property sets if/how a background image will be repeated.', 'Metabox Class', $this->textdomain ) . '</p>
		</div>
	...
	function mm_editor() {
		...
		if ( $type == 'slider' ) {
			...
			$output .= '<input type="text" id="mm-image-subtitle" name="subtitle" value="' . $options['title'] . '" />';
