...
function spectra_post_nav() {
	...
	$next_post = get_next_post_link( '%link', '<span class="meta-nav">' . __( 'Next Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
	$prev_post = get_previous_post_link( '%link', '<span class="meta-nav">' . __( 'Previous Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
		...
		<div class="nav-links">
			...
			if ( is_attachment() ) :
				...
				if ( ! $prev_post && $next_post ) {
					echo '<a></a>';
					echo get_next_post_link( '%link', '<span class="meta-nav">' . __( 'Next Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
				} else if ( $prev_post && ! $next_post ) {
					echo get_previous_post_link( '%link', '<span class="meta-nav">' . __( 'Previous Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
					echo '<a></a>';
				} else if ( $prev_post && $next_post  ) {
					echo get_previous_post_link( '%link', '<span class="meta-nav">' . __( 'Previous Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
					echo get_next_post_link( '%link', '<span class="meta-nav">' . __( 'Next Post', SPECTRA_THEME ) . '</span>' . __( '%title', SPECTRA_THEME ) );
...
function spectra_custom_post_nav() {
	...
	$prev_label = __( 'Pevious Post', SPECTRA_THEME );
	...
	if ( $post_type == 'spectra_events' || $post_type == 'spectra_portfolio' ) {
		...
		if ( $post_type == 'spectra_portfolio' ) {
			...
			$prev_label = __( 'Pevious Release', SPECTRA_THEME );
		}
		...
		if ( $post_type == 'spectra_events' ) {
			...
			$prev_label = __( 'Pevious Event', SPECTRA_THEME );
		}
