...
class R_Panel {
	...
	function admin_bar() {
		...
		$wp_admin_bar->add_menu(
			array( 
				'id' => 'r_theme_settings', 
				'title' => _x( 'Theme Options', 'Admin Panel Class', $this->textdomain ), 
				'href' => site_url() . '/wp-admin/admin.php?page=' . $this->args['page_name']
				)
			);
	...
	private function posts( $value ) {	
		...
		if ( post_type_exists( $value['post_type'] ) ) {
			...
			$posts = get_posts( array('post_type' => $value['post_type'] ) ); 
			if ( isset( $posts ) && is_array( $posts ) ) {
				foreach ( $posts as $post ) {
				  if ( $post->ID == $value['std'] ) $selected = 'selected="selected"';
				  else $selected = '';
	...
	private function add_image( $value ) {
		...
		if ( isset( $value['std'] ) && $value['std'] != '' && $theme_path == false ) {
				...
				if ( $by_id ) {
					...
					$image = $this->get_image( $value['std'] );
				}

				// If image exists
				if ( $image ) {
					echo '<img src="' . $this->img_resize($value['width'], $value['height'], $value['std'], 'c') . '" alt=" ' . $value['std'] . ' ">';
				} else {
					$display = 'block';
					$del_display = 'none';
				}
	...
	private function bg_generator_box() {
		...
		<label for="bg-repeat">' . _x('Repeat', 'Admin Panel Class', $this->textdomain ) . '</label>
	          ...
	          <p class="help-box">' . _x('The background-repeat property sets if/how a background image will be repeated.', 'Admin Panel Class', $this->textdomain ) . '</p>
		</div>
